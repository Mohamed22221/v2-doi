    ...
    return (
        <BackgroundRounded>
            <div className="p-6 space-y-8">
                {/* Basic Information */}
                <div>
                    <SectionHeader title={t('durationAuctions.details.basicInfo')} />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-5">
                        <InfoRow
                            label={t('durationAuctions.details.category')}
                            value={`${data?.categoryParent} → ${data?.categoryChild}`}
                        />
                        <InfoRow
                            label={t('durationAuctions.details.hall')}
                            value={data?.hall}
                        />
                        <InfoRow
                            label={t('durationAuctions.details.start')}
                            value={`${startDate} – ${startTime}`}
                        />
                        <InfoRow
                            label={t('durationAuctions.details.end')}
                            value={`${endDate} – ${endTime}`}
                        />
                        <div className="col-span-full">
                            <InfoRow
                                label={t('durationAuctions.details.description')}
                ...
                {/* Pricing & Stock */}
                <div>
                    <SectionHeader title={t('durationAuctions.details.pricingAndStock')} />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-5">
                        <InfoRow
                            label={t('durationAuctions.details.availableQuantity')}
                            value={data?.availableQuantity}
                        />
                        <InfoRow
                            label={t('durationAuctions.details.startingPrice')}
                            value={
                        ...
                        {data?.status === 'live' && (
                            <>
                                <InfoRow
                                    label={t('durationAuctions.details.currentHighestBid')}
                                    value={
                                        <span className="flex items-center gap-1 font-semibold text-lg">
                                ...
                                />
                                <InfoRow
                                    label={t('durationAuctions.details.timeRemaining')}
                        ...
                        {(data?.status === 'hidden' || data?.status === 'ended') && (
                            <>
                                <InfoRow
                                    label={t('durationAuctions.details.finalPrice')}
                                    value={
                                        <span className="flex items-center gap-1 font-semibold text-lg">
                                ...
                                />
                                <InfoRow
                                    label={t('durationAuctions.details.winner')}
                                    value={
                                        data?.winner ? (
                                            <Link
