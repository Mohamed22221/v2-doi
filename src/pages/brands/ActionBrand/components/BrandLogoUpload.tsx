...
const BrandLogoUpload = ({
    ...
    const beforeUpload = (fileList: FileList | null) => {
        ...
        if (fileList.length > 1) return t('brands.upload.oneOnly')
        ...
        if (!allowedFileType.includes(file.type))
            return t('brands.upload.invalidType')
        if (file.size > maxFileSize) return t('brands.upload.tooLarge')
            ...
            {({ field, form, meta }: FieldProps<string>) => {
                ...
                const handleUpload = async (files: File[]) => {
                    ...
                    try {
                        ...
                        if (!url) {
                            throw new Error(t('brands.upload.noUrl'))
                        }
                        ...
                        toast.push(
                            <Notification type="success">
                                {t('brands.upload.success')}
                            </Notification>,
                        )
                    } catch {
                        toast.push(
                            <Notification type="danger">
                                {t('brands.upload.failed')}
                            </Notification>,
                        )
                    }
                }
